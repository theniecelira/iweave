...
class _AdminBookingsScreenState extends State<AdminBookingsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(onPressed: () => context.read<BookingProvider>().loadAllBookings(),
        ...
        children: [
            ...
            child: SingleChildScrollView(
              ...
              child: Row(children: [
                ...
                ...BookingStatus.values.map((s) =>
                  _FilterChip(s.name[0].toUpperCase() + s.name.substring(1), _filter == s,
                    () => setState(() => _filter = s))),
              ]),
            ),
          ),
          ...
          Expanded(
            child: provider.isLoading
              ...
              : filtered.isEmpty
                ? const Center(child: Text('No bookings found', style: TextStyle(color: AppColors.textHint)))
                : ListView.separated(
                    ...
                    itemBuilder: (_, i) => _AdminBookingCard(
                      booking: filtered[i],
                      onStatusChange: (newStatus) => _changeStatus(context, filtered[i], newStatus),
                      onDelete: () => _confirmDelete(context, filtered[i]),
                      onTap: () => Navigator.pushNamed(context, '/booking-detail', arguments: filtered[i].id),
                    ),
                  ),
          ),
        ],
  ...
  void _changeStatus(BuildContext context, BookingModel booking, BookingStatus newStatus) async {
...
class _AdminBookingCard extends StatelessWidget {
  ...
  final VoidCallback onTap;
  const _AdminBookingCard({required this.booking, required this.onStatusChange, required this.onDelete, required this.onTap});
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ClipRRect(
              ...
              child: Image.network(booking.itemImage, width: 50, height: 50, fit: BoxFit.cover,
                errorBuilder: (_, __, ___) => Container(width: 50, height: 50, color: AppColors.tagBg)),
            ),
            const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(booking.itemName, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700), maxLines: 1, overflow: TextOverflow.ellipsis),
              Text('ID: ${booking.id}', style: const TextStyle(fontSize: 10, color: AppColors.textHint, fontFamily: 'monospace')),
              const SizedBox(height: 2),
              Text('User: ${booking.userId} · ${booking.guests} guests · ${AppFormatters.currency(booking.totalAmount)}',
                style: const TextStyle(fontSize: 11, color: AppColors.textSecondary)),
            ])),
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
              decoration: BoxDecoration(color: statusColor.withOpacity(0.12), borderRadius: BorderRadius.circular(8)),
              child: Text(booking.statusLabel, style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: statusColor)),
            ),
          ]),
          if (booking.checkIn != null)
            Padding(
              padding: const EdgeInsets.only(top: 8),
              child: Text('${AppFormatters.date(booking.checkIn!)} → ${booking.checkOut != null ? AppFormatters.date(booking.checkOut!) : "TBD"}',
  ...
  Color _color(BookingStatus s) {
    switch (s) {
      ...
      case BookingStatus.completed: return const Color(0xFF1565C0);
