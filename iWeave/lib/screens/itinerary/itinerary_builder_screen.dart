          ...
          GestureDetector(
            ...
            child: Container(
              ...
              child: Row(children: [
                ...
                Text(AppFormatters.date(_startDate),
          ...
          Wrap(
            ...
            children: List.generate(_days, (i) {
              ...
              return Container(
                ...
                child: Text(
                  'Day ${i + 1}  ${AppFormatters.dateShort(d)}',
                  style: const TextStyle(
                      fontSize: 12,
...
class _BuildStepState extends State<_BuildStep>
  ...
  void initState() {
    super.initState();
    final days =
        context.read<ItineraryProvider>().current?.numberOfDays ?? 1;
        ...
        Container(
          ...
          child: Row(children: [
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text(
                'Est. ${AppFormatters.currency(itinerary.totalEstimatedCost * itinerary.guests)}',
                style: const TextStyle(
                    fontSize: 16,
                    fontWeight: FontWeight.w800,
                    color: AppColors.primary),
            ...
            Expanded(
              child: AppButton(
                ...
                onPressed: itinerary.totalStops > 0
                    ? () =>
                        context.read<ItineraryProvider>().setStep(2)
                    : null,
              ),
            ),
          ]),
...
class _DayBuilder extends StatelessWidget {
          ...
          SliverPadding(
            ...
            sliver: SliverList(
              delegate: SliverChildBuilderDelegate(
                (_, i) => _StopTile(
                  stop: day.stops[i],
                  onRemove: () => provider.removeStop(
                      dayIndex: dayIndex, stopId: day.stops[i].id),
                ),
        ...
        if (day.stops.isEmpty)
          const SliverToBoxAdapter(
            child: Padding(
              ...
              child: Center(
                child: Text('Tap below to add stops for this day',
                    style: TextStyle(
          ...
          sliver: SliverList(
            delegate: SliverChildListDelegate([
              _AddSectionButton(
                ...
                label: 'Attractions',
                subtitle: '8 places in Basey',
                color: AppColors.primary,
                onTap: () => _showPicker(
                    context, _PickerType.attraction, dayIndex),
              ),
              _AddSectionButton(
                icon: Icons.restaurant_rounded,
                label: 'Restaurants & Cafés',
                subtitle: '4 local dining spots',
                color: AppColors.accent,
                onTap: () => _showPicker(
                    context, _PickerType.restaurant, dayIndex),
              ),
              _AddSectionButton(
                icon: Icons.hotel_rounded,
                label: 'Accommodations',
                subtitle: '4 stays in Basey',
                color: AppColors.success,
                onTap: () => _showPicker(
                    context, _PickerType.accommodation, dayIndex),
              ),
              _AddSectionButton(
                icon: Icons.auto_fix_high_rounded,
                label: 'Weaving Workshops',
                subtitle: '3 hands-on experiences',
                color: const Color(0xFF7B5EA7),
                onTap: () => _showPicker(
                    context, _PickerType.weaving, dayIndex),
              ),
            ]),
          ),
        ),
  ...
  void _showPicker(
      BuildContext context, _PickerType type, int dayIndex) {
...
class _StopPickerSheet extends StatelessWidget {
  ...
  const _StopPickerSheet(
      {required this.type, required this.dayIndex});
  ...
  Widget build(BuildContext context) {
    ...
    switch (type) {
      ...
      case _PickerType.restaurant:
        ...
        items = itineraryRestaurants;
        break;
      case _PickerType.accommodation:
        title = 'Accommodations';
        items = mockItineraryAccommodations;
        break;
      case _PickerType.weaving:
        title = 'Weaving Workshops';
        items = mockWeavingExperiences;
        break;
    }
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (_, i) {
                final item = items[i];
                final added =
                    provider.isStopAdded(item.id, dayIndex);
                return _PickerCard(
                  item: item,
                  type: type,
                  isAdded: added,
    ...
    switch (type) {
      case _PickerType.attraction:
        ...
        return ItineraryStop(
          id: uniqueId, type: StopType.attraction,
          timeSlot: TimeSlot.morning, itemId: a.id,
          name: a.name, imageUrl: a.imageUrl,
          cost: a.entryFee, icon: a.icon,
        );
      case _PickerType.restaurant:
        final r = item as RestaurantModel;
        return ItineraryStop(
          id: uniqueId, type: StopType.restaurant,
          timeSlot: TimeSlot.afternoon, itemId: r.id,
          name: r.name, imageUrl: r.imageUrl,
          cost: r.averageMealCost.toDouble(),
          icon: Icons.restaurant_rounded,
        );
      case _PickerType.accommodation:
        final acc = item as AccommodationModel;
        return ItineraryStop(
          id: uniqueId, type: StopType.accommodation,
          timeSlot: TimeSlot.evening, itemId: acc.id,
          name: acc.name, imageUrl: acc.imageUrl,
          cost: acc.pricePerNight, icon: Icons.hotel_rounded,
        );
      case _PickerType.weaving:
        final w = item as AttractionModel;
        return ItineraryStop(
          id: uniqueId, type: StopType.weaving,
          timeSlot: TimeSlot.morning, itemId: w.id,
          name: w.name, imageUrl: w.imageUrl,
          cost: w.entryFee, icon: w.icon,
        );
    }
  }
}
...
class _PickerCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (item is AttractionModel) {
      ...
      costLabel = a.entryFee == 0
          ? 'Free entry'
          : '₱${a.entryFee.toStringAsFixed(0)}/person';
          ...
          ClipRRect(
            ...
            child: Image.network(
              ...
              errorBuilder: (_, __, ___) => Container(
                  width: 90, height: 90, color: AppColors.tagBg),
            ),
          ),
          Expanded(
            child: Padding(
                ...
                children: [
                  ...
                  Row(children: [
                    ...
                    if (stop.cost > 0) ...[
                      ...
                      Text('₱${stop.cost.toStringAsFixed(0)}',
                          style: const TextStyle(
                              fontSize: 11,
                              color: AppColors.textSecondary)),
                    ],
...
class _ReviewStep extends StatelessWidget {
        ...
        Expanded(
          child: ListView(
            ...
            children: [
              Container(
                ...
                child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 10),
                      Row(children: [
                        _TripStat(Icons.calendar_today_rounded,
                            '${AppFormatters.dateShort(it.startDate)} – ${AppFormatters.date(it.endDate)}'),
                        const SizedBox(width: 16),
                        _TripStat(Icons.people_rounded,
                            '${it.guests} ${it.guests == 1 ? 'guest' : 'guests'}'),
                      ]),
                      const SizedBox(height: 4),
                      _TripStat(Icons.place_rounded,
                          '${it.totalStops} stops · ${it.numberOfDays} days'),
                    ]),
              ),
              const SizedBox(height: 20),
              ...List.generate(it.numberOfDays, (dayIdx) {
                final day = it.days[dayIdx];
                return Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Padding(
                      ...
                      child: Row(children: [
                        ...
                        Text(
                            'Day ${dayIdx + 1} · ${AppFormatters.date(day.date)}',
                            style: const TextStyle(
                                fontSize: 14,
                                fontWeight: FontWeight.w700)),
                        const Spacer(),
                    ...
                    if (day.stops.isEmpty)
                      Container(
                        ...
                        child: const Row(children: [
                          ...
                          SizedBox(width: 8),
                          Text('No stops added for this day',
                              style: TextStyle(
                                  fontSize: 13,
                                  color: AppColors.textHint)),
                        ]),
              ...
              const Divider(),
              const SizedBox(height: 12),
              _CostRow('Estimated entry fees & dining',
                  AppFormatters.currency(it.totalEstimatedCost * it.guests)),
              const SizedBox(height: 8),
              Container(
                padding: const EdgeInsets.all(14),
                decoration: BoxDecoration(
                ...
                child: Row(children: [
                  ...
                  Text(
                      AppFormatters.currency(
                          it.totalEstimatedCost * it.guests),
                      style: const TextStyle(
                          fontSize: 18,
                          fontWeight: FontWeight.w800,
                          color: AppColors.primary)),
              ...
              const SizedBox(height: 8),
              const Text(
                  '* Accommodation costs are per night, not per person.',
                  style: TextStyle(
  ...
  void _confirmBooking(BuildContext context, ItineraryProvider provider,
      ItineraryModel it) {
    showDialog(
      context: context,
      builder: (_) => AlertDialog(
        ...
        content: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Your trip "${it.tripName}" has been saved.',
                style: const TextStyle(
                    fontSize: 14, color: AppColors.textSecondary)),
            const SizedBox(height: 12),
            _ReviewStat(Icons.calendar_today_rounded,
                '${AppFormatters.dateShort(it.startDate)} – ${AppFormatters.date(it.endDate)}'),
            _ReviewStat(Icons.place_rounded,
                '${it.totalStops} stops planned'),
            _ReviewStat(Icons.people_rounded,
                '${it.guests} ${it.guests == 1 ? 'guest' : 'guests'}'),
            _ReviewStat(Icons.payments_rounded,
                'Est. ${AppFormatters.currency(it.totalEstimatedCost * it.guests)}'),
            const SizedBox(height: 12),
            Container(
              padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(
